-- Complete Setup: RLS Policies + Seed Data
-- Run this in Supabase SQL Editor

-- =============================================
-- STEP 1: DROP EXISTING POLICIES (if any)
-- =============================================
DROP POLICY IF EXISTS "Enable all access for anonymous users" ON customers;
DROP POLICY IF EXISTS "Enable all access for anonymous users" ON products;
DROP POLICY IF EXISTS "Enable all access for anonymous users" ON orders;
DROP POLICY IF EXISTS "Enable all access for anonymous users" ON materials;
DROP POLICY IF EXISTS "Enable all access for anonymous users" ON cargo_prices;
DROP POLICY IF EXISTS "Enable all access for anonymous users" ON sales_channels;
...
CREATE POLICY "Allow all for anon" ON cargo_prices FOR ALL TO anon USING (true) WITH CHECK (true);
CREATE POLICY "Allow all for anon" ON sales_channels FOR ALL TO anon USING (true) WITH CHECK (true);
...
('PARİLLA', 'PİKNİK MASASI 60*80 CM', 'Kamp Ürünleri', 10, 183.51, 39.60, 50, 28.2, 301.31, 282, 182.40, 7.12, 0.23, 227.17, 1000, 900, 1100, 931.50, 1067, 45, 'active'),
('PRİMAVERA', 'PİKNİK MASASI 45*60 CM', 'Kamp Ürünleri', 5, 116.29, 39.60, 50, 28.2, 234.09, 282, 182.40, 7.12, 0.29, 294.39, 1000, 900, 1100, 931.50, 1067, 55, 'active'),
('FUERTE TEKLİ', 'KATLANIR SANDALYE', 'Kamp Sandalyesi', 5, 140.79, 28.29, 50, 28.2, 247.27, 282, 182.40, 7.12, 0.28, 281.21, 1000, 900, 1100, 931.50, 1067, 60, 'active'),
('FUERTE İKİLİ', 'KATLANIR SANDALYE 2Lİ', 'Kamp Sandalyesi', 10, 281.58, 56.57, 50, 28.2, 416.35, 282, 182.40, 7.12, 0.11, 112.13, 1000, 900, 1100, 931.50, 1067, 40, 'active'),
('JARDİNERO', 'MASA + SANDALYE SETİ', 'Kamp Seti', 20, 465.09, 135.62, 50, 28.2, 678.90, 282, 182.40, 7.12, -0.15, -150.42, 1000, 900, 1100, 931.50, 1067, 8, 'inactive'),
...
('AHSAP CILALI ISKELET BAHÇE', 'MONTAJ', 'AD', 30.0, 300, 80),
('AHSAP CILALI ISKELET POPPY', 'MONTAJ', 'AD', 50.0, 180, 50),
('AHSAP CILALI ISKELET STARK', 'MONTAJ', 'TK', 300.0, 80, 20),
('AMBALAJ NAYLON PAT PAT', 'AMBALAJ', 'KG', 35.0, 500, 100),
('BALY 15 KG', 'BEYAZLAMA', 'KG', 55.0, 200, 50),
('BOYA PARLAK SIYAH', 'METALHANE', 'KG', 60.0, 150, 40),
('BOYA TEKSTURE SIYAH', 'METALHANE', 'KG', 40.0, 180, 50),
('CIRT 20 MM', 'TERZİHANE', 'M', 5.0, 1000, 200),
('CIVATA M6*15 MM', 'MONTAJ', 'AD', 1.0, 5000, 1000),
('CIVATA M6*20 MM', 'MONTAJ', 'AD', 1.0, 5000, 1000),
('CIVATA M8*40 MM', 'MONTAJ', 'AD', 1.5, 2000, 400),
('CIVATA M8*60 MM', 'MONTAJ', 'AD', 1.5, 1500, 300),
('METAL PROFIL 20*20*1 MM', 'MONTAJ', 'M', 18.0, 500, 100),
('METAL PROFIL 10*20*1 MM', 'MONTAJ', 'M', 15.0, 600, 120),
('SUNTALAM CEVIZ', 'MONTAJ', 'AD', 100.0, 200, 50),
('TAPA SIYAH 20*20 MM', 'MONTAJ', 'AD', 1.0, 3000, 500),
...
(85, 695.23), (90, 736.13), (95, 777.02), (100, 817.92);
...
('Mobilya Plus Ltd.', 'info@mobilyaplus.com', '02129998877', 'Sanayi Mah. No: 100', 'İstanbul', 'Başakşehir', 'corporate', 'Mobilya Plus Ltd. Şti.', 15, 85000.00, 'vip'),
...
-- =============================================
INSERT INTO orders (order_number, customer_name, products, total_amount, total_cost, profit, cargo_cost, sales_channel, status, payment_status, payment_method, shipping_address) VALUES
('SP-2024-00001', 'Ahmet Yılmaz', '[{"productName": "TORRE", "quantity": 1, "unitPrice": 2290}]'::jsonb, 2290, 1203.83, 804.17, 282, 'trendyol', 'delivered', 'paid', 'credit_card', 'Atatürk Cad. No: 15, Kadıköy, İstanbul'),
('SP-2024-00002', 'Mehmet Kaya', '[{"productName": "VİSİON", "quantity": 1, "unitPrice": 2990}]'::jsonb, 2990, 1535.00, 1173.00, 282, 'hepsiburada', 'shipped', 'paid', 'credit_card', 'Cumhuriyet Mah. No: 42, Çankaya, Ankara'),
('SP-2024-00003', 'Ayşe Demir', '[{"productName": "ORDENADO", "quantity": 2, "unitPrice": 2490}]'::jsonb, 4980, 2916.22, 1781.78, 282, 'n11', 'delivered', 'paid', 'bank_transfer', 'İstiklal Cad. No: 78, Konak, İzmir'),
('SP-2024-00004', 'Mobilya Plus Ltd.', '[{"productName": "İMPONENTE", "quantity": 3, "unitPrice": 3990}]'::jsonb, 11970, 7257.39, 3731.61, 327.17, 'direct', 'preparing', 'pending', 'bank_transfer', 'Sanayi Mah. No: 100, Başakşehir, İstanbul'),
('SP-2024-00005', 'Ali Öztürk', '[{"productName": "CUBO İKİLİ", "quantity": 2, "unitPrice": 1190}]'::jsonb, 2380, 790.26, 1307.74, 282, 'vivense', 'confirmed', 'paid', 'credit_card', 'Yeni Mah. Sok. No: 5, Muratpaşa, Antalya'),
('SP-2024-00006', 'Zeynep Arslan', '[{"productName": "SABİO", "quantity": 1, "unitPrice": 1890}]'::jsonb, 1890, 1078.13, 529.87, 282, 'bertaconcept', 'pending', 'pending', 'credit_card', 'Merkez Mah. No: 33, Selçuklu, Konya'),
('SP-2024-00007', 'Ev Dekor Tic.', '[{"productName": "GATİTO", "quantity": 5, "unitPrice": 1590}]'::jsonb, 7950, 4084.80, 3583.20, 282, 'direct', 'delivered', 'paid', 'bank_transfer', 'Ticaret Cad. No: 88, Ümraniye, İstanbul'),
('SP-2024-00008', 'Hüseyin Koç', '[{"productName": "MAGO", "quantity": 1, "unitPrice": 1990}]'::jsonb, 1990, 1154.00, 508.83, 327.17, 'koctas', 'shipped', 'paid', 'cash_on_delivery', 'Bahçelievler Mah. No: 12, Şahinbey, Gaziantep');

-- Done! You should now see data in all tables.
